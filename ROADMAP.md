...
- [ ] 品質段階(quality tiers)とデバイス自動判定 — 実装・静的検証済み。4段階preset+エミッタoverride、navigator.gpu limits/features、WebGL2 low、理由診断、構造variant分離。サンドボックス制約でGPU検収待ち
- [ ] 距離/視錐台カリング、significance manager(予算内で重要エフェクト優先) — 実装・静的検証済み。bounds/fade/pause、距離+画面占有+priority、安定tie、instance/particle予算。GPU検収待ち
