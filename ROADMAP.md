...
| 日付 | セッション成果 |
|---|---|
| 2026-07-12 | **M10バッチ2実装**: α半透明の三段構えを完成。coreは同一`VFXSystem`のalpha/premultipliedエミッタを既存`setCamera().viewMatrix`とlocal `sortCenter`でstable far-to-near粗ソートし、複数α+camera未設定を診断。`billboard/meshRenderer({ sorted:true })`はM2 compactionを変更せず別depth/index bufferへ展開し、2^n最遠番兵prefix→`P-aliveCount` fetch、(k,j)複数dispatch、depth+physical-index tieの決定的bitonicをWebGPU限定で実装(capacity≤65,536)。`@nachi/post`はthree r185 `RenderTarget(count:2)`+`mrt()`+attachment別blendによるMcGuire-Bavoil WBOIT(RGBA16F accum/R8 revealage)を追加しWebGL2を明示拒否。`/m10-sort/`は粗ソートcamera反転色、sorted depth/position/index readback CPU照合、2^n/同seed二回/kill混在、WBOIT既知2枚CPU値+順序不変、consoleClean/perf分離/offscreen 3-panel/WebGL2診断を常設。RFC§9.5/§13.7/§16、README/CLAUDE/post README更新。最終静的ゲートはformat/prettier-check/typecheck/lint/build/400 testsすべて緑。localhostはlisten EPERMのためページGPU実測はユーザー側検収へ。 |
