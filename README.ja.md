...
| パッケージ         | 用途                                                                                                                     |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `@nachi-vfx/core`     | 定義、パーティクルモジュール、コンパイラ、GPUスケジューラ、スケーラビリティ、シミュレーションキャッシュ、デバッガ、Grid2D/3D、近傍グリッド |
| `@nachi-vfx/three`    | Three.js WebGPUカーネル／ランタイムアダプター、リソースリゾルバー、パーティクル描画マテリアライザー                       |
| `@nachi-vfx/format`   | 厳密な`nachi-effect` v1 JSONスキーマ、シリアライザー／ローダー、マイグレーション、アセット継承                             |
| `@nachi-vfx/react`    | 薄いR3Fプロバイダー、フック、コンポーネントライフサイクル、`Object3D`アタッチメント                                       |
| `@nachi-vfx/timeline` | エフェクトローカルなシーケンシング、カメラシェイク、ヒットストップ、マーカー、mesh-fxライフサイクル                        |
| `@nachi-vfx/trails`   | GPUリボンとトレイル                                                                                                        |
| `@nachi-vfx/mesh-fx`  | プロシージャルなエフェクトジオメトリ、`fxMaterial`、Blender VAT再生                                                        |
| `@nachi-vfx/post`     | RenderPipelineのディストーション、放射状ブラー、ブルームプリセット、WebGPU WBOIT                                          |
| `@nachi-vfx/tsl-kit`  | スタンドアロンのThree.js TSLシェーダービルディングブロック                                                                |
...
Three.jsの型を公開するパッケージ(`@nachi-vfx/three`、`@nachi-vfx/tsl-kit`、`@nachi-vfx/mesh-fx`、`@nachi-vfx/trails`、
`@nachi-vfx/timeline`、`@nachi-vfx/post`、`@nachi-vfx/react`)は、TypeScriptプロジェクトにおいて別途公開されている
対応する型定義も必要とします:
...
マテリアライズの例は[`@nachi-vfx/react`](./packages/react/README.md)にあります。
...
初回npm公開と、その後のトークンレスTrusted Publishingへの移行を含むメンテナー向け手順は
[docs/releasing.md](./docs/releasing.md)に記載しています。
