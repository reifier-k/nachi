...
- 2026-07-10: ライブラリ名を **nachi** に決定(ユーザー選定。那智の滝由来=Niagara(滝)へのオマージュ。npm空き確認済み)。パッケージは `@nachi-vfx/*`(当初の `@vfx/*` プレースホルダから改名済み)、LICENSE holderは「nachi contributors」
- 2026-07-10: 監査プロトコル導入。全マイルストーンに監査項目(実装と別セッションで実施)+ M12後に最終監査FA(初回リリースゲート、当初1.0想定・現在0.1.0)。手順はROADMAP.md冒頭に定義
...
- 2026-07-12: **M12バッチ2(Grid2D+sim stages)設計確定**。sim stageはeffect要素`defineSimStage({target,phase,iterations,update})`(target=文字列キー、before-particles→particle stages→after-particlesの実行契約)。Grid2Dはstorage textureでなくpacked vec4 storage buffer(current+scratch 2本、チャネルをvec4レコードへ詰めSoA上限を節約)。iterationごとにstage dispatchとstate commitを独立submit(three実測知見準拠)。粒子→grid転送はu32固定小数(scale 4096)atomic加算→floatレゾルブ、grid→粒子はセル中心バイリニア。圧力はJacobi反復+勾配減算(低周波非収束・数値拡散・障害物/渦度閉じ込め/MacCormack非対応をRFC明記)。WebGL2は`NACHI_GRID2D_WEBGL2_UNSUPPORTED`明示拒否。**確定知見: gridカーネルは全て`index.lessThan(cells)`範囲ガード必須**(three r185はceil dispatchのみ生成、TintクランプバックエンドでOOB競合の移植性リスク)
- 2026-07-12: **M12バッチ4(neighbor grid+boids+PBD)設計確定**。空間ハッシュ=セル毎atomic u32 count+cell-major固定u32 slot(既定容量32、溢れはドロップ+計数+`NACHI_NEIGHBOR_GRID_CELL_OVERFLOW`)。bucket構築時にposition/velocityをsnapshotし、**合成モジュールの基底規定=自粒子状態はlive属性・近傍状態はsnapshot**(snapshot再基底化は先行モジュール書込を破棄する—gravity→boids順の重力消失をプローブ実証して確定)。近傍走査は**JS unrollでなくGPU動的TSL Loop**(z→y→x→slot、signed intオフセット、radius非依存のWGSL規模。radius 1の全展開=432 branchブロックがSwiftShader GPUプロセスを殺した実測から確定)。PBDはJacobi半分補正(反復ごとclear→bucket→constraint独立submit)。v1はNeighborGrid単一emitter所有、WebGL2はatomics不可のため明示拒否
- 2026-07-12: **M12バッチ5(エコシステム)設計確定**。`@nachi-vfx/react`はreact/R3F/three全peer、Provider+useVFXSystem+useEffectInstance+VFXEffectの最小面。useFrameでupdate駆動、definition/seed/priority変更=再spawn、parameter/transform/timeScale=live転送(coreの検証成功後にのみReact側へ記録=store-before-forward禁止)、cleanupはdetach→release。**確定知見: ラッパーはerror状態インスタンスへの操作転送を全経路でゲートする**(coreの穏やかな縮退設計を例外へ昇格させない)。リリースはchangesets独立バージョニング+`workspace:^`+release:dry(`--dry-run --offline --ignore-scripts`ハードコードで実publish経路なし)+全パッケージESM実importゲート。docsは軽量Vite静的サイト。Effekseerは調査のみ(RFC 002)=**実装は1.0以降へ延期と裁定**(.efkefcは安定interchange仕様でなくソースが実質仕様のため、version行列+fixture前提の変換製品として再検討)
- 2026-07-12: **M12バッチ1(JSON asset v1)設計確定**。フォーマット所有者はRFCどおり`@nachi-vfx/format`。封筒は`{format:'nachi-effect',version:1,effect}`、format所有構造はunknown fieldを拒否し、登録moduleのconfigだけをversion validatorへ委譲。ロード結果は必ず通常の正規化済み`EffectDefinition`。移行は明示1-step registry(v1→v1恒等)、asset emitter extendsはURI fragment解決後にM9 `defineEmitter(base,overrides)`へ委譲して意味論を一元化。inline関数/Three資源は拒否し、mesh-fx等のlive資源はJSON参照+外部bind。sim cache埋込みは将来version。
